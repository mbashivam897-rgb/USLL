...
| `USL_Financial_Model.xlsx` | **Live, fully formula-driven** workbook — 15 tabs: Cover, Sources & References, Assumptions, Revenue Buildup, Income Statement, Working Capital, PPE Schedule, Lease Schedule, Debt & Cash, Balance Sheet, Cash Flow, DCF, Comps, Historical Valuation, Scenarios. Every forecast figure is an **Excel formula linked across sheets** (no pasted numbers). |
...
## Key design choices
- **Live Excel formulas with cell linkages** — the workbook is auditable end-to-end. Colour convention:
  - 🔵 **Blue** = hard-coded inputs (historical actuals + forecast assumptions)
  - ⚫ **Black** = calculations (formulas referencing the same sheet)
  - 🟢 **Green** = links/references to another worksheet (`='Sheet'!Cell`)
  - The **Sources & References** tab documents the provenance of every blue figure.
...
- **Balance sheet balances to ₹0 every year by construction**: equity rolls forward (PAT − dividends); all operating items are driver-based; **treasury (cash + deposits + current investments) is the balancing item**; the cash-flow statement is derived from BS/IS movements and **ties exactly to the change in treasury**.
- **Non-circular**: treasury income is earned on the *opening* balance, so the model is a clean forward recursion (no iterative calculation needed). Validated by evaluating the workbook: balance check = 0 and cash-flow tie = 0 for all years; DCF value/share = ₹743 with a live WACC×g sensitivity grid.
